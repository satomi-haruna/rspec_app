...
describe '投稿のテスト' do
  let!(:book) { create(:book,title:'hoge',body:'body') } # let!(:book)データの作成を行い、bookとして使用できる
  describe 'トップ画面(root_path)のテスト' do
    before do
      visit root_path # visit=指定したパスへの画面遷移
    end
    context '表示の確認' do
    it 'トップ画面(root_path)に一覧ページへのリンクが表示されているか' do
      expect(page).to have_link "", href: books_path # ecpect(page)で現在開いているページ全体について調べる
    end
...
describe "一覧画面のテスト" do
  ...
  context '一覧の表示とリンクの確認' do
    it "bookの一覧表示(tableタグ)と投稿フォームが同一画面に表示されているか" do
      expect(page).to have_selector 'table' # 特定のタグが存在するか
      expect(page).to have_field 'book[title]' # 指定した値のフォームが存在するか
